...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/Nirvana-Chain/nirvanad-DNSseeder)

DNSSeeder exposes a list of known peers to any new peer joining the
Nirvana network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the
`Nirvanad` node specified with the `-s` flag and listen for `addr`
...
file, so on subsequent start ups the `Nirvanad` node specified with `-s`
...
* Launch a `Nirvanad` node for the DNSSeeder to connect to
...
```bash
git clone https://github.com/Nirvana-Chain/nirvanad-DNSseeder ${GOPATH}/src/github.com/Nirvana-Chain/nirvanad-DNSseeder
cd ${GOPATH}/src/github.com/Nirvana-Chain/nirvanad-DNSseeder
go install .
```
...
`Nirvanad` instances, set the following DNS records:
